...
typedef struct bst {
	...
	bst* lchild=NULL, * rchild=NULL;/*树的左右孩子*/
}bst,*bstree;/*查找表和根节点*/
...
/*初始化节点*/
/*开辟空间并给数据域赋值，返回空间地址*/
bstree Init_Binary_Sort_Tree(bstree active,int goal) {
	active = (bst*)malloc(sizeof(bst));
		...
		while (b) {
			if (keys[i] < b->key.word) {/*小*/
				if (b->lchild) b = b->lchild;/*后面有就继续比较*/
					...
					b->lchild = (bst*)malloc(sizeof(bst));
					b = b->lchild;
					b->key.word = keys[i];
					break;
				}
				...
				if (b->rchild) b = b->rchild;/*后面有继续比较*/
					...
					b->rchild = (bst*)malloc(sizeof(bst));
					b = b->rchild;
					b->key.word = keys[i];
					break;
				}
...
int main() {
	...
	active->lchild = Init_Binary_Sort_Tree(active->lchild,99);
	active = active->lchild;
	active->rchild = Init_Binary_Sort_Tree(active->rchild,110);
	active = active->rchild;
	active->lchild = Init_Binary_Sort_Tree(active->lchild, 105);
	active = active->lchild;
	active = quite;/*回到根节点*/
	active->rchild = Init_Binary_Sort_Tree(active->rchild, 250);/*右子树赋值*/
	active = active->rchild;
	active->lchild = Init_Binary_Sort_Tree(active->lchild, 200);
	active->rchild = Init_Binary_Sort_Tree(active->rchild, 300);
	active = active->lchild;
	active->rchild = Init_Binary_Sort_Tree(active->rchild, 230);
	active = active->rchild;
	active->lchild = Init_Binary_Sort_Tree(active->lchild, 216);
	active = quite;/*回到根节点*/
	/*算法查找*/
	keytype key1 = 105;/*目标值*/
	bstree end = Binary_Sort_Tree_Search(quite, key1);
	if (!end)printf("没找着");
	...
	bstree b = (bst*)malloc(sizeof(bst));
	if (!b)return -1;
